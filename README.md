...
└── commands/             # Click CLI commands (auto-discovered by `agent_chat_app cmd …`)
...
The generated project ships a Click CLI exposed as `agent_chat_app` (after `make install`):

```bash
agent_chat_app server run --reload          # dev server
agent_chat_app db upgrade                   # apply migrations
agent_chat_app db migrate -m "message"      # create new migration
agent_chat_app user create-admin            # interactive admin creation
agent_chat_app rag-ingest <path> -c docs    # ingest local files
agent_chat_app rag-search "query" -c docs   # semantic search
agent_chat_app rag-collections              # list collections
```

Run `make help` for a categorized list, or `agent_chat_app --help` for full CLI docs.
...
agent_chat_app rag-ingest /path/to/docs/ --collection documents --recursive

# Semantic search
agent_chat_app rag-search "your query" --collection documents
```
...
*Generated with [Full-Stack AI Agent Template](https://github.com/vstorm-co/full-stack-ai-agent-template) vDYNAMIC.*
