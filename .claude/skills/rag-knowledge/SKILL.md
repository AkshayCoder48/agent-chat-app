...
```bash
uv run agent_chat_app cmd rag-ingest ./docs/ --collection docs --recursive   # ingest files/folder
uv run agent_chat_app cmd rag-search "your question" --collection docs        # semantic search
uv run agent_chat_app cmd rag-collections                                     # list collections
uv run agent_chat_app cmd rag-stats                                           # chunk/vector counts
uv run agent_chat_app cmd rag-drop <collection> --yes                         # delete a collection
```
...
```bash
uv run agent_chat_app cmd rag-sources                  # list
uv run agent_chat_app cmd rag-source-add               # add (interactive)
uv run agent_chat_app cmd rag-source-sync --all        # trigger a sync
```
