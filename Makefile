...
dev:
	...
	docker compose -f docker-compose.dev.yml exec -T app agent_chat_app db upgrade
	@echo ""
...
seed:
	...
	@if docker compose -f docker-compose.dev.yml exec -T app \
		agent_chat_app user list 2>/dev/null \
...
stage:
	...
	docker compose -f docker-compose.yml exec -T app agent_chat_app db upgrade
...
prod:
	...
	@sleep 5
	docker compose --env-file backend/.env -f docker-compose.prod.yml exec -T app agent_chat_app db upgrade
...
db-init: docker-db
	...
	@sleep 8
	cd backend && uv run agent_chat_app db migrate -m "initial" || true
	cd backend && uv run agent_chat_app db upgrade
	@echo ""
	@echo "✅ Database initialized!"

db-migrate:
	@read -p "Migration message: " msg; \
	uv run --directory backend agent_chat_app db migrate -m "$$msg"

db-upgrade:
	uv run --directory backend agent_chat_app db upgrade

db-downgrade:
	uv run --directory backend agent_chat_app db downgrade

db-current:
	uv run --directory backend agent_chat_app db current

db-history:
	uv run --directory backend agent_chat_app db history

# === Server ===
run:
	uv run --directory backend agent_chat_app server run --reload

run-prod:
	uv run --directory backend agent_chat_app server run --host 0.0.0.0 --port 8000

routes:
	uv run --directory backend agent_chat_app server routes

# === Users ===
create-admin:
	@echo "Creating admin user..."
	uv run --directory backend agent_chat_app user create-admin

user-create:
	uv run --directory backend agent_chat_app user create

user-list:
	uv run --directory backend agent_chat_app user list
...
docker-db:
	...
	@echo "   Connection: postgresql://postgres:postgres@localhost:5432/agent_chat_app"
...
help:
	@echo ""
	@echo "agent-chat-app - Available Commands"
	@echo "======================================"
	@echo ""
	...
	@echo ""
	@echo "RAG:"
	@echo "  uv run agent_chat_app rag-ingest <path> -c <collection>  Ingest files"
	@echo "  uv run agent_chat_app rag-search <query> -c <collection>  Search"
	@echo "  uv run agent_chat_app rag-collections                     List collections"
	@echo "  uv run agent_chat_app rag-sources                         List sync sources"
	@echo "  uv run agent_chat_app rag-source-add                      Add sync source"
	@echo "  uv run agent_chat_app rag-source-sync <id>                Trigger sync"
	@echo ""
