...
**agent-chat-app** - FastAPI application generated with [Full-Stack AI Agent Template](https://github.com/vstorm-co/full-stack-ai-agent-template).

**Stack:** FastAPI + Pydantic v2, PostgreSQL (async via asyncpg)
, JWT + API Key auth, PydanticAI, RAG (milvus), Next.js 15 (i18n)
...
# RAG
uv run agent_chat_app rag-collections
uv run agent_chat_app rag-ingest /path/to/file.pdf --collection docs
uv run agent_chat_app rag-search "query" --collection docs

# Sync Sources
uv run agent_chat_app cmd rag-sources
uv run agent_chat_app cmd rag-source-add
uv run agent_chat_app cmd rag-source-sync
```
